...
After solving a problem, **proactively check if AGENTS.md or skills can be improved.** If you learn something that would prevent future mistakes, update the relevant document immediately. Don't wait to be asked.

### No Project Structure in Docs

**NEVER add directory trees, file listings, or project structure to KNOWLEDGE.md or AGENTS.md.** This information goes stale instantly and contradicts the code. The only namespace to mention is the entry point: `com.blockether.styrmann.main`.
