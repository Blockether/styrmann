...
import type { Task, TaskType, Workspace } from '@/lib/types';
    ...
    async function loadData() {
      try {
        const [workspaceRes, tasksRes] = await Promise.all([
          ...
          fetch(`/api/tasks?workspace_id=${slug}`),
        ]);
    ...
    return (
      <div className="min-h-screen bg-mc-bg flex items-center justify-center">
        <div className="flex flex-col items-center">
          ...
          <p className="text-mc-text-secondary">Loading Pareto analysis...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg sm:text-xl font-semibold truncate">Pareto Analysis</h1>
              <p className="text-xs sm:text-sm text-mc-text-secondary truncate">
                {workspace ? workspace.name : 'Workspace'} - Impact vs Effort Matrix
              </p>
            </div>
          </div>
        </div>
                        ...
                        {cellTasks.length > 0 && (
                          <div className="relative w-full h-full">
                            {cellTasks.slice(0, 5).map((task, i) => (
                              <Link
                                key={task.id}
                                href={`/workspace/${slug}?taskId=${task.id}`}
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {unscoredTasks.map((task) => (
                <Link
                  key={task.id}
                  href={`/workspace/${slug}?taskId=${task.id}`}
                  className="p-3 bg-mc-bg border border-mc-border rounded-lg hover:border-mc-accent/50 transition-colors"
                >
