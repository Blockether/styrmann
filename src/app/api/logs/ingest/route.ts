...
export async function POST(request: NextRequest) {
  try {
    ...
    const insert = db.prepare(`
      INSERT OR IGNORE INTO agent_logs (id, agent_id, session_id, task_id, role, content, content_hash, workspace_id, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const touchSession = db.prepare('UPDATE sessions SET updated_at = ? WHERE session_id = ?');
    ...
    const insertAll = db.transaction(() => {
      for (const entry of entries) {
        ...
        if (!entry.content_hash || !entry.session_id) continue;

        // Resolve task_id: use provided value, or look up from sessions
        let taskId = entry.task_id || null;
        if (!taskId) {
          const session = lookupTaskId.get(entry.session_id) as { task_id: string } | undefined;
        ...
        touchSession.run(entry.created_at || new Date().toISOString(), entry.session_id);
      }
    });
