    ...
    (it "appends multiple distinct deliverables"
      (with-temp-conn [conn (temp-conn)]
        (let [ctx (execution-context/make-context conn)
              ...
              tid      (:task/id task-rec)
              tool-ctx {:ctx ctx}]
          (sut/record-deliverable tool-ctx {:task-id tid :title "First"  :description "D1" :status "pending"})
          (let [result (sut/record-deliverable tool-ctx {:task-id tid :title "Second" :description "D2" :status "done"})]
            (expect (= true (:ok? result)))
    ...
    (it "updates existing deliverable by title — idempotent upsert"
      (with-temp-conn [conn (temp-conn)]
        (let [ctx (execution-context/make-context conn)
              ...
              tid      (:task/id task-rec)
              tool-ctx {:ctx ctx}]
          (sut/record-deliverable tool-ctx {:task-id tid :title "Report" :description "v1" :status "pending"})
          (let [result (sut/record-deliverable tool-ctx {:task-id tid :title "Report" :description "v2 updated" :status "done"})]
            (expect (= true (:ok? result)))
            (expect (= 1 (count (:deliverables result))))
    ...
    (it "transitions implementing -> testing"
      (with-temp-conn [conn (temp-conn)]
        (let [ctx (execution-context/make-context conn)
              ...
              task-rec (make-task conn {:ticket ticket :workspace workspace} nil)
              tid      (:task/id task-rec)]
          (sut/update-task-status {:ctx ctx} {:task-id (str tid) :status "implementing"})
          (let [result (sut/update-task-status {:ctx ctx} {:task-id (str tid) :status "testing"})]
            (expect (= true (:ok? result)))
