    ...
    (it "creates planner, implementer, reviewer, explorer, verifier agents"
        (with-temp-conn [conn (temp-conn)]
          (let [ctx (execution-context/make-context conn)]
            ...
            (session/sync-tool-definitions! ctx (registry/list-tools))
            (let [agents (sut/ensure-all-agents! ctx)]
    ...
    (it "is idempotent — second call returns same agents"
        (with-temp-conn [conn (temp-conn)]
          (let [ctx (execution-context/make-context conn)]
            (registry/register-default-tools!)
            (session/sync-tool-definitions! ctx (registry/list-tools))
            (let [first-run (sut/ensure-all-agents! ctx)
                  second-run (sut/ensure-all-agents! ctx)]
...
(defdescribe find-agent-by-type-test
  (describe "finds agents by type keyword"
    (it "returns the planner agent"
        (with-temp-conn [conn (temp-conn)]
          (let [ctx (execution-context/make-context conn)]
            (registry/register-default-tools!)
            (session/sync-tool-definitions! ctx (registry/list-tools))
            (sut/ensure-all-agents! ctx)
            (let [planner (sut/find-agent-by-type ctx :agent.type/planner)]
    ...
    (it "rejects invalid agent types"
        (with-temp-conn [conn (temp-conn)]
          (let [ctx (execution-context/make-context conn)]
            (expect (try (sut/find-agent-by-type ctx :agent.type/invalid)
                         false
